...
final class Domain
{
    ...
    /**
     * Raport dnia (Wspomnienia, v4): pełne zestawienie danych z danego dnia.
     * Zwraca datę, dzień życia, wpisy (chronologicznie), pasma snu i agregaty
     * summary. Używane przez /api/memory (generowanie na żądanie + cache).
     */
    public static function dayMemory(array $entries, string $date): array
    {
        $day = [];
        foreach ($entries as $e) {
            ...
            $day[] = [
                'time' => $e['time'], 'type' => $e['type'],
                'label' => self::isMilkType($e['type']) ? self::milkTypeLabel($e['type']) : self::memoryLabel($e['type']),
                'ml' => $e['ml'], 'piersLeftMin' => $e['piersLeft'], 'piersRightMin' => $e['piersRight'],
                'lineIndex' => $e['lineIndex'] ?? -1,
            ];
        }
        ...
        foreach ($day as $e) {
            switch ($e['type']) {
                case 'KARMIENIE': $s['feedingCount']++; $s['piersLeftMin'] += $e['piersLeftMin']; $s['piersRightMin'] += $e['piersRightMin']; break;
                case 'MLEKO_MATKI': $s['milkMl'] += $e['ml']; $s['motherMilkMl'] += $e['ml']; break;
                case 'MLEKO_MODYFIKOWANE': $s['milkMl'] += $e['ml']; $s['modifiedMilkMl'] += $e['ml']; break;
                case 'MLEKO_MIESZANE': $s['milkMl'] += $e['ml']; $s['mixedMilkMl'] += $e['ml']; break;
                case 'PIELUCHA_MOKRA': $s['diaperWet']++; break;
                case 'PIELUCHA_BRUDNA': $s['diaperDirty']++; break;
                case 'ODCIAGANIE': $s['pumpingMl'] += $e['ml']; break;
                case 'WITAMINA_D': $s['vitaminD'] = true; break;
                case 'WAGA': $s['weightG'] = $e['ml']; break;
                case 'KAPIEL': $s['bathCount']++; break;
                case 'SEN_START': $openStart = $e['time']; break;
                case 'SEN_STOP':
                    if ($openStart !== null) {
                        $span = self::minutesBetweenTimes($openStart, $e['time']);
                        if ($span !== null) $sleepSpans[] = ['from' => $openStart, 'to' => $e['time'], 'min' => $span];
                        $openStart = null;
                    }
                    break;
            }
        }
        $sleepDay = 0; $sleepNight = 0;
        foreach ($sleepSpans as $sp) {
            $h = (int)substr($sp['from'], 0, 2);
            if ($h >= Config::SLEEP_NIGHT_START_HOUR || $h < Config::SLEEP_NIGHT_END_HOUR) $sleepNight += $sp['min'];
            else $sleepDay += $sp['min'];
        }
        $s['sleepDayMin'] = $sleepDay; $s['sleepNightMin'] = $sleepNight; $s['napCount'] = count($sleepSpans);

        return [
            'date' => $date,
            'dayOfLife' => self::dayOfLifeForDate($date),
            'entries' => $day,
    ...
    private static function minutesBetweenTimes(string $t1, string $t2): ?int
    {
        $m1 = self::timeToMinutes($t1); $m2 = self::timeToMinutes($t2);
        if ($m1 === null || $m2 === null) return null;
        $d = $m2 - $m1;
        if ($d < 0) $d += 1440; // sen przez polnoc
        return $d;
    }
    ...
    {
        return ['feedingCount' => 0, 'milkMl' => 0, 'motherMilkMl' => 0, 'modifiedMilkMl' => 0, 'mixedMilkMl' => 0,
                'piersLeftMin' => 0, 'piersRightMin' => 0, 'diaperWet' => 0, 'diaperDirty' => 0, 'pumpingMl' => 0,
                'vitaminD' => false, 'weightG' => 0, 'bathCount' => 0, 'sleepDayMin' => 0, 'sleepNightMin' => 0, 'napCount' => 0];
    }
}
