...
<script>
const state={data:null,view:'home',activeDay:null,milkMother:true,milkModified:false,editFeedLine:null,editMilkHad:false};
...
function refreshMilkKindButtons(){const bm=$('milkMother'),bx=$('milkModified');bm.classList.toggle('selected',!!state.milkMother);bx.classList.toggle('selected',!!state.milkModified);bm.textContent=(state.milkMother?'✓ ':'')+'MATKI';bx.textContent=(state.milkModified?'✓ ':'')+'MODYFIKOWANE'}
...
function openEditFeeding(feed,milk){state.pumpMode=false;state.editFeedLine=feed.lineIndex;state.editMilkHad=!!milk;['timeLabel','entryTime','nudgeBox','bottleToggle'].forEach(id=>$(id).classList.remove('hidden'));$('nursingBox').classList.add('hidden');$('quickNotice').classList.add('hidden');setText('extraTitle','RODZAJ MLEKA (MOŻESZ OBA → MIESZANE)');$('kindRow').classList.remove('hidden');$('bottleLabel').classList.remove('hidden');setText('formTitle',`EDYCJA KARMIENIA — ${feed.time}`);const day=state.activeDay||(state.data&&state.data.nowIso?state.data.nowIso.slice(0,10):'');$('entryTime').value=`${day}T${feed.time}`;const d=state.data||{};$('milkMl').min=d.milkMinMl||20;$('milkMl').max=d.milkMaxMl||200;$('milkMl').step=d.milkStepMl||10;
// Prefill rodzaju/ilosci wg istniejacego mleka (MIESZANE => oba zaznaczone).
if(milk){const t=milk.type||'';state.milkMother=(t==='MLEKO_MATKI'||t==='MLEKO_MIESZANE');state.milkModified=(t==='MLEKO_MODYFIKOWANE'||t==='MLEKO_MIESZANE');$('milkMl').value=milk.ml||(d.milkDefaultMl||60);state.bottleOpen=true}else{state.milkMother=true;state.milkModified=false;$('milkMl').value=d.milkDefaultMl||60;state.bottleOpen=false}
$('milkRemoveBtn').classList.toggle('hidden',!milk);refreshMilkKindButtons();updateBottle();setText('milkAmount',`${$('milkMl').value} ml`);setText('formNotice','');openFormModal()}
...
$('entryForm').addEventListener('submit',async event=>{event.preventDefault();const notice=$('formNotice');notice.className='notice';setText('formNotice','Zapisywanie…');try{
  ...
  if(state.editFeedLine!=null){
    ...
    else{if(!state.milkMother&&!state.milkModified){notice.className='notice error';setText('formNotice','Zaznacz rodzaj mleka albo usuń mleko.');return}body.set('milkMl',$('milkMl').value);body.set('milkMother',state.milkMother?'1':'0');body.set('milkModified',state.milkModified?'1':'0')}
