...
<main class="app">
  ...
  <div id="weightModal" class="modal" role="presentation" aria-hidden="true">
    ...
    <div class="card modal-dialog">
      ...
      <div class="chart-legend" style="margin-top:14px"><span class="dot" style="background:var(--moss)"></span>Zmierzona <span class="dot" style="background:var(--berry)"></span>Mediana WHO <span class="dot" style="background:#bcd9c2"></span>Oczekiwany zakres (min–max)</div>
...
let maxG=0,minBand=1e9;bandPts.forEach(b=>{if(b.max>maxG)maxG=b.max;if(b.min<minBand)minBand=b.min});pts.forEach(p=>{if(p.g>maxG)maxG=p.g;if(p.g<minBand)minBand=p.g});maxG=Math.ceil((maxG+300)/500)*500;const minG=Math.max(0,Math.floor((minBand-300)/500)*500);
