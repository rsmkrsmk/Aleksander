...
.sleep-state{text-align:center;margin:6px 0 2px}.sleep-big{font-size:1.02rem;font-weight:850;color:var(--ink);margin-bottom:10px}.sleep-bar{border-radius:14px;padding:10px 12px;font-size:.82rem;font-weight:850;color:#fff;background:var(--muted)}.sleep-bar.ok{background:#3f9d55}.sleep-bar.warn{background:#D98B5F}.sleep-bar.bad{background:var(--danger)}.sleep-window{margin-top:8px;font-size:.74rem;font-weight:700;color:var(--muted)}
.sleep-balance{margin-top:14px}.sleep-bal-grid{display:grid;grid-template-columns:1fr auto;gap:6px 14px;font-size:.84rem;margin-top:8px}.sleep-bal-grid .bk{color:var(--muted);font-weight:700}.sleep-bal-grid .bv{text-align:right;font-weight:850;color:var(--ink)}.sleep-bal-grid .bv.ok{color:var(--moss)}
...
.diag-card.hidden{display:none}.diag-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px 14px;font-size:.8rem}.diag-grid .dk{color:var(--muted);font-weight:700}.diag-grid .dv{text-align:right;font-weight:850;color:var(--ink)}.diag-grid .dv.ok{color:var(--moss)}.diag-grid .dv.bad{color:var(--danger)}.diag-toggle{display:inline-flex;align-items:center;gap:6px;cursor:pointer}.diag-toggle input{width:18px;height:18px;accent-color:var(--moss)}
...
<main class="app">
  ...
  <p class="since-bath" id="sinceBath" style="text-align:center;font-size:.75rem;color:var(--muted);margin:2px 0 4px"></p>
  ...
  <nav class="actions" aria-label="Główne działania">
    ...
    <button id="homeSleepBtn" data-action="sleep-open" style="background:#6E5FA6">SEN</button>
    <button data-action="other">INNE</button>
    <button data-action="bath" style="background:#14B8A6">KĄPIEL</button>
  ...
  <div id="chart5Modal" class="modal" role="presentation" aria-hidden="true">
    ...
    <div class="card modal-dialog">
      ...
      <div class="chart-legend"><span class="dot" style="background:var(--moss)"></span>Matki <span class="dot" style="background:linear-gradient(180deg,var(--moss),var(--berry))"></span>Mieszane <span class="dot" style="background:var(--berry)"></span>Modyfikowane <span class="dot" style="background:var(--acorn)"></span>Karmienia</div>
      ...
      <div id="rhythm3"></div>
      <h3 class="chart-subtitle" style="margin-top:22px">KAPIEL — 5 DNI</h3>
      <div class="chart-legend" style="margin:2px 0 8px"><span class="dot" style="background:#14B8A6"></span>Kapiel</div>
      <div class="chart-wrap" id="bathChart"></div>
      <p class="chart-note" id="bathSummary" style="margin:2px 0 8px"></p>
    </div>
  </div>
  ...
  <div id="otherModal" class="modal" role="presentation" aria-hidden="true">
    ...
    <div class="card modal-dialog">
      ...
      <div class="actions"><button class="milk wide" data-action="diaper">PIELUCHA</button><button class="wide" data-action="pumping">ODCIAG POKARMU</button><button type="button" class="wide" data-action="bath" style="background:#14B8A6;color:#fff">KĄPIEL</button></div>
    </div>
  </div>

  <div id="bathModal" class="modal" role="presentation" aria-hidden="true">
    <div class="modal-backdrop" data-action="home"></div>
    <div class="card modal-dialog">
      <div class="modal-head"><h2>KĄPIEL</h2><button class="small muted" data-action="home">ZAMKNIJ</button></div>
      <div class="weight-entry">
        <label for="bathDate">DATA KĄPIELI</label>
        <div class="weight-row" style="justify-content:center">
          <input id="bathDate" type="date" value="" style="flex:1;text-align:center">
        </div>
        <p class="notice" style="font-size:.8rem;color:var(--muted);text-align:center;margin-top:6px">Bez godziny — zapisujemy samą datę.</p>
  ...
  <div id="sleepModal" class="modal" role="presentation" aria-hidden="true">
    <div class="modal-backdrop" data-action="home"></div>
    <div class="card modal-dialog">
      <div class="modal-head"><h2>SEN</h2><button class="small muted" data-action="home">ZAMKNIJ</button></div>
      <div class="sleep-state" id="sleepStateCard">
        <div class="sleep-big" id="sleepBig">—</div>
        <div class="sleep-bar" id="sleepBar"><span id="sleepBarText">—</span></div>
        <div class="sleep-window" id="sleepWindow">—</div>
      </div>
      <button type="button" class="wide" id="sleepToggleBtn" data-action="sleep" style="background:#6E5FA6;margin-top:12px">ZASNĄŁ</button>
      <div class="card sleep-balance" id="sleepBalance">
        <div class="section-title">BILANS DNIA</div>
        <div class="sleep-bal-grid" id="sleepBalGrid"></div>
      </div>
      <p class="chart-note">Okno czuwania i cele snu są orientacyjne (wg wieku, na podstawie Napper). Każde dziecko rozwija się we własnym tempie.</p>
    </div>
  </div>
  ...
  <form class="card form-card modal-dialog" id="entryForm" role="dialog" aria-modal="true" aria-labelledby="formTitle">
    ...
    <div class="extra-milk hidden" id="extraMilkOptions"><h3 id="extraTitle">RODZAJ MLEKA (MOŻESZ OBA → MIESZANE)</h3><div class="milk-kind" id="kindRow"><button type="button" class="selected" id="milkMother">✓ MATKI</button><button type="button" class="milk" id="milkModified">MODYFIKOWANE</button></div><label for="milkMl" id="bottleLabel">ILOŚĆ MLEKA</label><div class="amount" id="milkAmount">60 ml</div><input id="milkMl" type="range" min="20" max="200" step="10" value="60"><div class="range-ends"><span>20 ml</span><span>200 ml</span></div><div class="dual-ml hidden" id="dualMlField"><div class="dual-ml-row"><button type="button" class="mlpick mother" id="mlPickMother" data-milkpick="mother"><span class="mlpick-k">MATKI</span><span class="mlpick-v" id="mlMotherVal">60 ml</span></button><button type="button" class="mlpick modified" id="mlPickModified" data-milkpick="modified"><span class="mlpick-k">MODYFIKOWANE</span><span class="mlpick-v" id="mlModifiedVal">60 ml</span></button></div><div class="dual-ml-sum" id="dualMlSum">Razem 120 ml</div></div><button type="button" class="muted small hidden" id="milkRemoveBtn" data-action="milk-remove" style="width:100%;margin-top:12px">USUŃ MLEKO Z TEGO KARMIENIA</button></div>
    <div class="mlpop-wrap hidden" id="mlPop"><div class="mlpop-card"><h3 id="mlPopTitle">ILOŚĆ MLEKA MATKI</h3><div class="amount" id="mlPopVal">60 ml</div><input id="mlPopRange" type="range" min="20" max="200" step="10" value="60"><div class="range-ends"><span>20 ml</span><span>200 ml</span></div><div class="mlpop-btns"><button type="button" class="muted" id="mlPopMinus">− 10</button><button type="button" class="muted" id="mlPopPlus">+ 10</button><button type="button" class="feed" id="mlPopOk">OK</button></div></div></div>
...
<script>
const state={data:null,view:'home',activeDay:null,milkMother:true,milkModified:false,editFeedLine:null,editMilkHad:false,milkMotherMl:60,milkModifiedMl:60,mlPopKind:null};
const $=id=>document.getElementById(id);
const modals=['calendarModal','detailModal','chartModal','diaperModal','chart5Modal','otherModal','weightModal','sleepModal','bathModal'];
...
function renderDiag(data){const g=$('diagGrid');if(!g||$('diagCard').classList.contains('hidden'))return;const row=(k,v,cls)=>`<div class="dk">${k}</div><div class="dv ${cls||''}">${v}</div>`;const up=data.uptimeSec||0;const d=Math.floor(up/86400),h=Math.floor((up%86400)/3600),m=Math.floor((up%3600)/60);let html='';
  ...
  // Przelacznik powiadomien Telegram o oknie snu (klikalny checkbox -> /api/setting).
  html+=`<div class="dk">Sen: powiadomienia Telegram</div><div class="dv"><label class="diag-toggle"><input type="checkbox" id="sleepTgChk" ${data.sleepTelegram?'checked':''}> <span>${data.sleepTelegram?'wł.':'wył.'}</span></label></div>`;
  g.innerHTML=html;
  const chk=$('sleepTgChk');
  if(chk)chk.addEventListener('change',async()=>{try{await request('/api/setting',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({key:'sleepTelegram',value:chk.checked?'1':'0'})});await refresh()}catch(e){alert(e.message)}});
}
...
function renderChart(cal){const MAX=Math.max(...cal.map(d=>d.milkMl),1);const milkChart=$('milkChart');milkChart.replaceChildren();cal.forEach(d=>{const u=document.createElement('div');u.className='bar-unit';const g=document.createElement('div');g.className='bar-pair';const mh=d.motherMilkMl/MAX*100;const mixh=(d.mixedMilkMl||0)/MAX*100;const modh=d.modifiedMilkMl/MAX*100;if(mh>0){const b=document.createElement('div');b.className='bar mother';b.style.height=mh+'%';b.title=d.motherMilkMl+' ml matki';g.append(b)}if(mixh>0){const b=document.createElement('div');b.className='bar mixed';b.style.height=mixh+'%';b.title=d.mixedMilkMl+' ml mieszane';g.append(b)}if(modh>0){const b=document.createElement('div');b.className='bar modified';b.style.height=modh+'%';b.title=d.modifiedMilkMl+' ml modyf.';g.append(b)}const v=document.createElement('div');v.className='bar-value';v.textContent=d.milkMl||'0';v.style.fontSize='.58rem';u.append(v,g);const feed=document.createElement('div');feed.style.fontSize='.65rem';feed.style.fontWeight='800';feed.style.color='var(--acorn)';feed.textContent='●'+d.feedingCount;u.append(feed);milkChart.append(u)});const extra=$('extraTable');const anyMix=cal.some(d=>(d.mixedMilkMl||0)>0);let html='<table class="chart-table"><tr><th>Dzien</th><th>Karm.</th><th>Matki</th><th>Mod.</th>'+(anyMix?'<th>Miesz.</th>':'')+'<th>Suma</th></tr>';cal.forEach(d=>{const label=d.label.split(' - ')[0];html+=`<tr><td>${label}</td><td>${d.feedingCount}</td><td>${d.motherMilkMl} ml</td><td>${d.modifiedMilkMl} ml</td>`+(anyMix?`<td>${d.mixedMilkMl||0} ml</td>`:'')+`<td class="ok">${d.milkMl||0} ml</td></tr>`});html+='</table>';extra.innerHTML=html}
function renderBathChart(cal){const host=$('bathChart');if(!host)return;host.replaceChildren();const maxB=Math.max(1,...cal.map(d=>d.bathCount||0));cal.forEach(d=>{const col=document.createElement('div');col.className='bar-unit';const g=document.createElement('div');g.className='bar-pair';if((d.bathCount||0)>0){const b=document.createElement('div');b.className='bar bath';b.style.height=((d.bathCount||0)/maxB*100)+'%';b.title=(d.bathCount||0)+' kapieli';g.append(b)}const v=document.createElement('div');v.className='bar-value';v.textContent=d.bathCount||'0';v.style.fontSize='.58rem';col.append(v,g);const lab=document.createElement('div');lab.style.fontSize='.55rem';lab.style.color='var(--muted)';lab.textContent=(d.label||'').split(' - ').pop().slice(0,5);col.append(lab);host.append(col)});const total=cal.reduce((a,d)=>a+(d.bathCount||0),0);const sb=$('bathSummary');if(sb)sb.textContent=total===0?'Brak kapieli w ostatnich 5 dniach':`Lacznie ${total} kapieli w 5 dni`}
...
function openPumping(){state.pumpMode=true;state.editFeedLine=null;state.editMilkHad=false;$('milkRemoveBtn').classList.add('hidden');setText('formTitle','ODCIAGANIE MLEKA');['timeLabel','entryTime','nudgeBox','quickNotice','bottleToggle','nursingBox'].forEach(id=>$(id).classList.add('hidden'));$('extraMilkOptions').classList.remove('hidden');setText('extraTitle','ODCIAGANIE — ILOŚĆ ML');$('kindRow').classList.add('hidden');$('bottleLabel').classList.add('hidden');$('dualMlField').classList.add('hidden');$('milkMl').classList.remove('hidden');{const en=$('milkMl').nextElementSibling;if(en&&en.classList.contains('range-ends'))en.classList.remove('hidden')}$('milkAmount').classList.remove('hidden');const d=state.data||{};$('milkMl').min=d.minMl||10;$('milkMl').max=d.maxMl||120;$('milkMl').step=5;$('milkMl').value=d.defaultMl||30;$('entryTime').value=dateTimeInput(state.data&&state.data.nowIso);setText('milkAmount',`${$('milkMl').value} ml`);setText('formNotice','');openFormModal()}
function toggleMilkKind(which){if(which==='mother')state.milkMother=!state.milkMother;else state.milkModified=!state.milkModified;refreshMilkKindButtons()}
function refreshMilkKindButtons(){const bm=$('milkMother'),bx=$('milkModified');bm.classList.toggle('selected',!!state.milkMother);bx.classList.toggle('selected',!!state.milkModified);bm.textContent=(state.milkMother?'✓ ':'')+'MATKI';bx.textContent=(state.milkModified?'✓ ':'')+'MODYFIKOWANE';refreshMilkAmountUi()}
// Oba rodzaje (mieszane) => dwa pola ilosci (matki/modyfikowane) + popover; jeden => suwak; brak => oba ukryte.
function refreshMilkAmountUi(){if(state.pumpMode)return;const both=state.milkMother&&state.milkModified;const one=(state.milkMother||state.milkModified)&&!both;const single=$('milkMl'),ends=single.nextElementSibling;single.classList.toggle('hidden',!one);if(ends&&ends.classList.contains('range-ends'))ends.classList.toggle('hidden',!one);$('milkAmount').classList.toggle('hidden',!one);$('dualMlField').classList.toggle('hidden',!both);if(both)syncDualLabels()}
function syncDualLabels(){setText('mlMotherVal',`${state.milkMotherMl} ml`);setText('mlModifiedVal',`${state.milkModifiedMl} ml`);setText('dualMlSum',`Razem ${(+state.milkMotherMl||0)+(+state.milkModifiedMl||0)} ml`)}
function updateBottle(){const open=state.bottleOpen;$('extraMilkOptions').classList.toggle('hidden',!open);setText('bottleToggle',open?'SZCZEGÓŁY — UKRYJ':'SZCZEGÓŁY');if(open)refreshMilkAmountUi()}
function openForm(date){state.pumpMode=false;state.editFeedLine=null;state.editMilkHad=false;$('milkRemoveBtn').classList.add('hidden');['timeLabel','entryTime','nudgeBox','quickNotice','bottleToggle','nursingBox'].forEach(id=>$(id).classList.remove('hidden'));setText('extraTitle','RODZAJ MLEKA (MOŻESZ OBA → MIESZANE)');$('kindRow').classList.remove('hidden');$('bottleLabel').classList.remove('hidden');setText('formTitle',date?`DODAJ KARMIENIE — ${dateLabel(date)}`:'NOWE KARMIENIE');$('entryTime').value=date?`${date}T12:00`:dateTimeInput(state.data&&state.data.nowIso);const d=state.data||{};$('milkMl').min=d.milkMinMl||20;$('milkMl').max=d.milkMaxMl||200;$('milkMl').step=d.milkStepMl||10;$('milkMl').value=d.milkDefaultMl||60;$('piersL').value=0;$('piersR').value=0;state.bottleOpen=false;state.milkMother=true;state.milkModified=false;state.milkMotherMl=d.milkDefaultMl||60;state.milkModifiedMl=d.milkDefaultMl||60;refreshMilkKindButtons();updateBottle();setText('milkAmount',`${$('milkMl').value} ml`);setText('formNotice','');openFormModal()}
// Edycja istniejacego karmienia (mleko + godzina). feed = wpis KARMIENIE (z lineIndex),
// milk = sparowany wpis MLEKO_* dla tej samej godziny (lub null). Minuty piersi nie
// sa edytowane — pole PIERS chowamy. Wysylka -> /api/update-feeding (feedLine=feed.lineIndex).
function openEditFeeding(feed,milks){milks=Array.isArray(milks)?milks:(milks?[milks]:[]);state.pumpMode=false;state.editFeedLine=feed.lineIndex;state.editMilkHad=milks.length>0;['timeLabel','entryTime','nudgeBox','bottleToggle'].forEach(id=>$(id).classList.remove('hidden'));$('nursingBox').classList.add('hidden');$('quickNotice').classList.add('hidden');setText('extraTitle','RODZAJ MLEKA (MOŻESZ OBA → MIESZANE)');$('kindRow').classList.remove('hidden');$('bottleLabel').classList.remove('hidden');setText('formTitle',`EDYCJA KARMIENIA — ${feed.time}`);const day=state.activeDay||(state.data&&state.data.nowIso?state.data.nowIso.slice(0,10):'');$('entryTime').value=`${day}T${feed.time}`;const d=state.data||{};$('milkMl').min=d.milkMinMl||20;$('milkMl').max=d.milkMaxMl||200;$('milkMl').step=d.milkStepMl||10;
// Prefill rodzaju/ilosci wg istniejacego mleka (MIESZANE => oba zaznaczone).
const dv=d.milkDefaultMl||60;let motherMl=0,modifiedMl=0,mixedMl=0;milks.forEach(m=>{const t=m.type;const v=+m.ml||0;if(t==='MLEKO_MATKI')motherMl+=v;else if(t==='MLEKO_MODYFIKOWANE')modifiedMl+=v;else if(t==='MLEKO_MIESZANE')mixedMl+=v;else motherMl+=v});if(mixedMl>0){if(motherMl===0)motherMl=mixedMl;if(modifiedMl===0)modifiedMl=mixedMl}if(milks.length){state.milkMother=motherMl>0;state.milkModified=modifiedMl>0;state.bottleOpen=true}else{state.milkMother=true;state.milkModified=false;state.bottleOpen=false}state.milkMotherMl=motherMl>0?motherMl:dv;state.milkModifiedMl=modifiedMl>0?modifiedMl:dv;$('milkMl').value=(state.milkModified&&!state.milkMother)?state.milkModifiedMl:state.milkMotherMl;
$('milkRemoveBtn').classList.toggle('hidden',!milks.length);refreshMilkKindButtons();updateBottle();setText('milkAmount',`${$('milkMl').value} ml`);setText('formNotice','');openFormModal()}
...
$('milkMl').addEventListener('input',()=>{const v=+$('milkMl').value||0;setText('milkAmount',`${v} ml`);if(!state.pumpMode){if(state.milkMother&&!state.milkModified)state.milkMotherMl=v;else if(state.milkModified&&!state.milkMother)state.milkModifiedMl=v}});
$('bottleToggle').addEventListener('click',()=>{state.bottleOpen=!state.bottleOpen;updateBottle()});$('milkMother').addEventListener('click',()=>toggleMilkKind('mother'));$('milkModified').addEventListener('click',()=>toggleMilkKind('modified'));
// Mleko mieszane: pola + popover wyboru ilosci
function openMlPop(kind){state.mlPopKind=kind;const d=state.data||{};const r=$('mlPopRange');r.min=d.milkMinMl||20;r.max=d.milkMaxMl||200;r.step=d.milkStepMl||10;const cur=kind==='mother'?state.milkMotherMl:state.milkModifiedMl;r.value=cur;setText('mlPopTitle',kind==='mother'?'ILOŚĆ MLEKA MATKI':'ILOŚĆ MLEKA MODYFIKOWANEGO');setText('mlPopVal',`${cur} ml`);$('mlPop').classList.remove('hidden')}
function closeMlPop(){$('mlPop').classList.add('hidden');state.mlPopKind=null}
function mlPopApply(){const v=+$('mlPopRange').value||0;if(state.mlPopKind==='mother')state.milkMotherMl=v;else if(state.mlPopKind==='modified')state.milkModifiedMl=v;syncDualLabels()}
$('mlPickMother').addEventListener('click',()=>openMlPop('mother'));$('mlPickModified').addEventListener('click',()=>openMlPop('modified'));
...
$('entryForm').addEventListener('submit',async event=>{event.preventDefault();const notice=$('formNotice');notice.className='notice';setText('formNotice','Zapisywanie…');try{
  // Tryb EDYCJI karmienia -> /api/update-feeding (mleko + godzina, in-place).
  if(state.editFeedLine!=null){
    const body=new URLSearchParams({feedLine:String(state.editFeedLine),when:$('entryTime').value});
    if(!state.bottleOpen){body.set('milkRemove','1')}
    else{if(!state.milkMother&&!state.milkModified){notice.className='notice error';setText('formNotice','Zaznacz rodzaj mleka albo usuń mleko.');return}body.set('milkMotherMl',String(state.milkMother?(+state.milkMotherMl||0):0));body.set('milkModifiedMl',String(state.milkModified?(+state.milkModifiedMl||0):0))}
    const result=await request('/api/update-feeding',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});
    notice.className='notice ok';setText('formNotice',result.message||'Zapisano edycję.');await refresh();
    setTimeout(()=>{const returnDay=state.activeDay;closeFormModal();returnDay?openDay(returnDay,state.detailLabel):clearPanels()},550);
    return;
  }
  let body;if(state.pumpMode){body=new URLSearchParams({type:'ODCIAGANIE',when:$('entryTime').value,ml:$('milkMl').value})}else{const extra=state.bottleOpen;if(extra&&!state.milkMother&&!state.milkModified){notice.className='notice error';setText('formNotice','Zaznacz rodzaj mleka (matki i/lub modyfikowane).');return}body=new URLSearchParams({type:'KARMIENIE',when:$('entryTime').value,ml:'0',extraMilk:extra?'1':'0',lewaMin:Number($('piersL').value)||0,prawaMin:Number($('piersR').value)||0});if(extra){body.set('milkMotherMl',String(state.milkMother?(+state.milkMotherMl||0):0));body.set('milkModifiedMl',String(state.milkModified?(+state.milkModifiedMl||0):0))}}const result=await request('/api/entry',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});notice.className='notice ok';setText('formNotice',result.message||'Wpis zapisany.');await refresh();setTimeout(()=>{const returnDay=state.activeDay;closeFormModal();if(returnDay&&!state.pumpMode)openDay(returnDay,state.detailLabel);else clearPanels()},550)}catch(error){notice.className='notice error';setText('formNotice',error.message)}});
// --- SEN (Napper): modal ze stanem, oknem czuwania i bilansem dnia ---
...
function openSleep(){show('sleepModal');renderSleep()}
function renderSleep(){
  const d=state.data||{};
  const btn=$('sleepToggleBtn');
  if(btn){btn.textContent=d.sleepInProgress?'OBUDZIŁ SIĘ':'ZASNĄŁ';btn.style.background=d.sleepInProgress?'#E0A93F':'#6E5FA6'}
  const big=$('sleepBig'),bar=$('sleepBar'),barTxt=$('sleepBarText'),win=$('sleepWindow');
  bar.className='sleep-bar';
  const st=d.sleepState||'brak';
  if(st==='spi'){
    big.textContent='Śpi'+(d.sleepSinceMin>=0?` — ${fmtDur(d.sleepSinceMin)}`:'');
    bar.classList.add('ok');barTxt.textContent='Sen w toku';
    win.textContent='';
    ...
    if(st==='czuwa'){bar.classList.add('ok');barTxt.textContent=`Za wcześnie — okno drzemki ~${s}–${e}`}
    else if(st==='okno'){bar.classList.add('warn');barTxt.textContent=`OKNO DRZEMKI TERAZ (do ~${e})`}
    else{bar.classList.add('bad');barTxt.textContent='Przekroczone okno (ryzyko przemęczenia)'}
    win.textContent=`Okno czuwania wg wieku: ${fmtDur(d.wakeWindowMinMin)} – ${fmtDur(d.wakeWindowMaxMin)}`;
  }else{
    big.textContent='Brak danych snu';barTxt.textContent='Gdy dziecko zaśnie — dotknij ZASNĄŁ';win.textContent='';
  }
  // Bilans dnia
  const g=$('sleepBalGrid');
...
function openBath(){const inp=$('bathDate');inp.value=isoToday();setText('bathNotice','');show('bathModal')}
async function saveBath(){const val=$('bathDate').value;const notice=$('bathNotice');notice.className='notice';if(!val||!/^\d{4}-\d{2}-\d{2}$/.test(val)){notice.className='notice error';setText('bathNotice','Wybierz date kapieli.');return}setText('bathNotice','Zapisywanie…');try{const r=await request('/api/event',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({type:'KAPIEL',when:val+'T12:00'})});notice.className='notice ok';setText('bathNotice',r.message||'Zapisano kapiel.');await refresh();setTimeout(()=>{closeBathModal()},600)}catch(e){notice.className='notice error';setText('bathNotice',e.message)}}
function closeBathModal(){const m=$('bathModal');m.classList.remove('open');m.setAttribute('aria-hidden','true');document.body.classList.remove('modal-open');clearPanels()}
...
document.addEventListener('click',async event=>{const action=event.target.closest('[data-action]')?.dataset.action;if(!action)return;if(action==='new-feed')openForm();if(action==='quick-feed'){if(await postEvent('KARMIENIE'))clearPanels()}if(action==='quick-feed-close'){if(await postEvent('KARMIENIE')){closeFormModal();clearPanels()}}if(action==='diaper')show('diaperModal');if(action==='import')$('importFile').click();if(action==='toggle-diag'){const c=$('diagCard');c.classList.toggle('hidden');if(!c.classList.contains('hidden')&&state.data)renderDiag(state.data)}if(action==='send-backup'){try{const r=await request('/api/send-backup',{method:'POST'});setText('status',r.message||'Backup wysylany.');$('status').classList.remove('error')}catch(e){setText('status',e.message);$('status').classList.add('error')}}if(action==='diaper-wet'){if(await postEvent('PIELUCHA_MOKRA'))clearPanels()}if(action==='diaper-dirty'){if(await postEvent('PIELUCHA_BRUDNA'))clearPanels()}if(action==='pumping')openPumping();if(action==='vitamin')await postEvent('WITAMINA_D');if(action==='undo'){renderSummary()}if(action==='calendar')show('calendarModal');if(action==='chart'){show('chartModal');state.summaryExtra=0;renderSummary()}if(action==='chart5'){show('chart5Modal');if(state.data&&state.data.calendar){renderChart(state.data.calendar);renderBathChart(state.data.calendar)}renderAnalysis()}if(action==='other'){show('otherModal')}if(action==='sleep-open'){openSleep()}if(action==='sleep'){const t=(state.data&&state.data.sleepInProgress)?'SEN_STOP':'SEN_START';if(await postEvent(t))renderSleep()}if(action==='weight'){openWeight()}if(action==='w-minus'){const w=$('weightG');w.value=Math.max(2000,(Number(w.value)||3700)-10)}if(action==='w-plus'){const w=$('weightG');w.value=Math.min(15000,(Number(w.value)||3700)+10)}if(action==='w-save'){saveWeight()}if(action==='bath'){openBath()}if(action==='bath-save'){saveBath()}if(action==='home')clearPanels();if(action==='back-calendar')show('calendarModal');if(action==='day-feed'&&state.activeDay)openForm(state.activeDay);if(action==='cancel-form'){setText('formNotice','');const returnDay=state.activeDay;closeFormModal();returnDay?openDay(returnDay):clearPanels()}if(action==='minus15')nudge(-5);if(action==='plus15')nudge(5);if(action==='milk-remove'){state.bottleOpen=false;state.milkMother=false;state.milkModified=false;closeMlPop();$('milkRemoveBtn').classList.add('hidden');refreshMilkKindButtons();updateBottle();setText('formNotice','Mleko zostanie usunięte z tego karmienia po zapisaniu.')}});
